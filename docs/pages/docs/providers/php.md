...
If an `NIXPACKS_PHP_ROOT_DIR` variable is passed, that will be used as the server root.
If an `NIXPACKS_PHP_FALLBACK_PATH` variable is passed, that will be used as a fallback for the server - for instance, if your app uses `index.php` as a router, you would set this variable to `/index.php`.
